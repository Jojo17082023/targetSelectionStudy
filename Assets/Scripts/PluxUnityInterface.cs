...
namespace Assets.Scripts
{
    public class PluxUnityInterface : MonoBehaviour
    {
        ...
        [Tooltip("Diese List Variable vom Typ double bekommt in jedem Frame die aktuellen TKEO-konvertierten emg werte als. Wir bilden immer den average -> convertedvalues.Average().")]
        ...
        [Tooltip("Dieser Wert ist am Anfang Null und speichert den hoechsten wert von convertedvalues.Average().")]
                        ...
                        if (MultiThreadList[VisualizationChannel].Count >= 0)
                        {
                            ...
                            else if (FirstPlot == false)
                            {
                                ...
                                if (UpdatePlotFlag && pacakgeOfData != null)
                                {
                                    ...
                                    if (trigger == true) {
                                        if (convertedvalues.Average() > threshold_l)
                                        {
                                            Debug.Log("PluxUnityInterface thresh high true");
                                            trigger = true;
                                        }
                                        else
                                        {
                                            Debug.Log("PluxUnityInterface thresh high false");
                                            trigger = false;
                                        }
                                    } else if(trigger == false){
                                        if(convertedvalues.Average() > threshold_h)
                                        {
                                            Debug.Log("PluxUnityInterface thresh low true");
                                            trigger = true;
                                        }
                                        else
                                        {
                                            Debug.Log("PluxUnityInterface thresh low false");
                                            trigger = false;
                                        }
                                    }
        ...
        // Wir rufen das hier auf nachdem wir gecheckt haben ob eine trigger aktiv wurde oder nicht.
